...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('pending', 'running', 'passed', 'failed', 'cancelled', 'healing', name='runstatus'), nullable=False, server_default='pending'),
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('pending', 'approved', 'rejected', 'auto_approved', name='healingstatus'), nullable=False, server_default='pending'),
        sa.Column('created_at', sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
    ...
    op.create_table(
        ...
        sa.Column('order', sa.Integer, nullable=False),
        sa.Column('status', sa.Enum('pending', 'running', 'passed', 'failed', 'skipped', 'healing', 'healed', name='stepstatus'), nullable=False, server_default='pending'),
        sa.Column('started_at', sa.DateTime(timezone=True), nullable=True),
        sa.Column('finished_at', sa.DateTime(timezone=True), nullable=True),
        sa.Column('duration_ms', sa.Integer, nullable=True),
    ...
    # Deferred circular FK: healing_candidates -> step_executions
    op.create_foreign_key('fk_healing_candidates_step_execution', 'healing_candidates', 'step_executions', ['step_execution_id'], ['id'], ondelete='CASCADE')
    ...
    op.create_table(
        ...
        sa.Column('impact', sa.Enum('critical', 'serious', 'moderate', 'minor', name='accessibilityimpact'), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('strategy', sa.Enum('css', 'xpath', 'text', 'role', 'testId', 'id', 'name', 'placeholder', 'label', name='locatorstrategy'), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('steps', postgresql.JSONB, nullable=False, server_default='[]'),
        sa.Column('outcome', sa.Enum('success', 'failure', 'partial', name='episodeoutcome'), nullable=False),
...
def downgrade() -> None:
    ...
    # Drop enum types
    op.execute("DROP TYPE IF EXISTS failure_pattern_enum CASCADE")
    op.execute("DROP TYPE IF EXISTS episodeoutcome CASCADE")
    op.execute("DROP TYPE IF EXISTS locatorstrategy CASCADE")
    op.execute("DROP TYPE IF EXISTS accessibilityimpact CASCADE")
    op.execute("DROP TYPE IF EXISTS healingstatus CASCADE")
