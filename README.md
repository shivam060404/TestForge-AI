...
```
├── backend/          # FastAPI backend and execution worker
...
- **AI runtime**: versioned prompts, provider routing, bounded retries/timeouts,
  structured output guardrails, memory-first grounding, usage/cost telemetry,
...
PostgreSQL is mandatory for every environment. The API rejects SQLite or
non-`asyncpg` database URLs at startup.
