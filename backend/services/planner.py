...
from core.ai.prompts import TEST_PLANNER_PROMPT, render_planner_user_prompt
from core.ai.runtime import llm_router
...
class Planner:
    ...
    def __init__(self):
        self.system_prompt = TEST_PLANNER_PROMPT.system
    ...
    async def _llm_generate(
        ...
        try:
            result = await llm_router.complete(
                ...
                user=render_planner_user_prompt(intent, context),
                prompt_version=TEST_PLANNER_PROMPT.version,
            )
            if result is None:
                return self._fallback_generate(intent)
            steps_data = parse_and_validate_steps(result.content)
        ...
        except (GuardrailViolation, json.JSONDecodeError) as e:
            logger.warning("llm_output_rejected", error=str(e), intent=intent[:50])
            return self._fallback_generate(intent)
